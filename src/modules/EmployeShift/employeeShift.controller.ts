...
const assignShiftToEmployee = catchAsync(
...
const unassignShiftFromEmployee = catchAsync(
  async (req: Request, res: Response) => {
    const user = req.employee as AuthEmployee;
...
// Get all employees assigned to a specific shift
const getEmployeesByShiftId = catchAsync(
  async (req: Request, res: Response) => {
    const { shiftId } = req.params;
...
const getActiveEmployeeShift = catchAsync(
  async (req: Request, res: Response) => {
    const { employeeId } = req.params;
    const employeeShift = await employeeShiftService.getActiveEmployeeShift(
      employeeId
    );
    ...
    if (!employeeShift) {
      throw new ApiError(
        httpStatus.NOT_FOUND,
        "No active shift assignment found for this employee"
      );
    }
...
const getEmployeeShiftHistory = catchAsync(
  async (req: Request, res: Response) => {
    const { employeeId } = req.params;
    const employeeShifts = await employeeShiftService.getEmployeeShiftHistory(
      employeeId
    );
...
const calculateWorkingHours = catchAsync(
  async (req: Request, res: Response) => {
    const user = req.employee as AuthEmployee;
    const { employeeId } = req.params;
...
const bulkAssignShiftToEmployees = catchAsync(
  async (req: Request, res: Response) => {
    const user = req.employee as AuthEmployee;
...
const bulkUnassignShiftFromEmployees = catchAsync(
  async (req: Request, res: Response) => {
    const user = req.employee as AuthEmployee;
