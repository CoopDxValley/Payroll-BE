...
const overtimeManagementService = {
  ...
  async getTodaysOvertime(filters: any, companyId: string): Promise<EnhancedOvertimeSession[]> {
  ...
  // Get weekly overtime
  async getWeeklyOvertime(filters: any, companyId: string): Promise<EnhancedOvertimeSession[]> {
  ...
  async getMonthlyOvertime(filters: any, companyId: string): Promise<EnhancedOvertimeSession[]> {
  ...
  // Get yearly overtime
  async getYearlyOvertime(filters: any, companyId: string): Promise<EnhancedOvertimeSession[]> {
  ...
  async getOvertimeByDate(filters: SingleDateQuery, companyId: string): Promise<EnhancedOvertimeSession[]> {
  ...
  async getOvertimeSummary(filters: DateRangeQuery, companyId: string): Promise<EnhancedOvertimeResponse> {
    ...
    const totalOvertimeHours = sessions.reduce((sum, s) => sum + s.overtimeDuration, 0) / 60;
    const averageOvertimePerEmployee = uniqueEmployees > 0 ? totalOvertimeHours / uniqueEmployees : 0;

    const pendingOvertime = sessions.filter(s => s.overtimeStatus === 'PENDING').length;
    const approvedOvertime = sessions.filter(s => s.overtimeStatus === 'APPROVED').length;
    const rejectedOvertime = sessions.filter(s => s.overtimeStatus === 'REJECTED').length;

    const overtimeByType = {
      UNSCHEDULED: sessions.filter(s => s.overtimeType === 'UNSCHEDULED').length,
      EARLY_ARRIVAL: sessions.filter(s => s.overtimeType === 'EARLY_ARRIVAL').length,
      LATE_DEPARTURE: sessions.filter(s => s.overtimeType === 'LATE_DEPARTURE').length,
      EXTENDED_SHIFT: sessions.filter(s => s.overtimeType === 'EXTENDED_SHIFT').length,
      HOLIDAY_WORK: sessions.filter(s => s.overtimeType === 'HOLIDAY_WORK').length,
      REST_DAY_WORK: sessions.filter(s => s.overtimeType === 'REST_DAY_WORK').length,
    };
  ...
  async createOvertimeTable(data: CreateOvertimeTableRequest): Promise<IOvertimeTableWithRelations> {
    ...
    // Calculate duration
    const duration = calculateDurationMinutes(punchInDateTime, punchOutDateTime);
  ...
  async getOvertimeTables(filters: OvertimeTableFilters): Promise<IOvertimeTableWithRelations[]> {
  ...
  async updateOvertimeTable(id: string, data: UpdateOvertimeTableRequest): Promise<IOvertimeTableWithRelations> {
    ...
    if (data.punchIn) {
      const punchInDateTime = new Date(`${overtime.date.toISOString().split('T')[0]}T${data.punchIn}`);
      updateData.punchIn = punchInDateTime;
      updateData.punchInTime = data.punchIn;
    }

    if (data.punchOut) {
      const punchOutDateTime = new Date(`${overtime.date.toISOString().split('T')[0]}T${data.punchOut}`);
  ...
  async updateOvertimeStatus(id: string, data: UpdateOvertimeStatusRequest): Promise<IOvertimeTableWithRelations> {
