...
const restDaySchema = baseDaySchema.extend({
  dayType: z.literal("REST_DAY"),
  startTime: z.string().default("00:00:00"),
  endTime: z.string().default("00:00:00"),
  breakTime: z.number().default(0),
  gracePeriod: z.number().default(0),
});

const workingDaySchema = baseDaySchema.extend({
...
const patternDaySchema = z.discriminatedUnion("dayType", [
  restDaySchema,
  workingDaySchema,
]);
...
const createShift = {
  body: z
    .object({
      ...
      patternDays: z.array(patternDaySchema).optional(),
    })
    .refine(
      (data) => {
        if (data.shiftType === "FIXED_WEEKLY") {
          if (!data.patternDays || data.patternDays.length !== 7) return false;

          const dayNumbers = data.patternDays
      ...
      {
        message:
          "FIXED_WEEKLY shifts must have exactly 7 pattern days covering all days (1-7, Monday-Sunday)",
        path: ["patternDays"],
      }
    ),
};
