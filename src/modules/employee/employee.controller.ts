...
export const uploadEmployee = catchAsync(async (req, res) => {
  ...
  const workbook = new ExcelJS.Workbook();
  await workbook.xlsx.load(new Uint8Array(req.file.buffer) as any);
  ...
  const employees: { row: number; data: CreateEmployeeInput }[] = [];
  ...
  sheet.eachRow((row, rowNumber) => {
    ...
    try {
      ...
      employees.push({
        row: rowNumber,
        data: { personalInfo, payrollInfo, emergencyContacts },
      });
    } catch (err: any) {
      invalidRows.push({ row: rowNumber, error: err.message });
    }
  });
  ...
  const { successes, failures } = await employeeService.bulkCreateEmployees(
    employees,
    authEmployee.companyId
  );
